...
#[tokio::main]
pub async fn resolve_versions(vers: Vec<String>, os: String) -> Vec<Rversion> {
...
async fn resolve_release(client: &reqwest::Client, os: &String) -> Rversion {
    let url;
    if os == "macos" {
    ...
    let dlurl = get_download_url(&v, os);
    Rversion { version: v.to_string(), url: dlurl }
}

async fn resolve_devel(client: &reqwest::Client, os: &String) -> Rversion {
    let url = DEVEL_VERSION_URI.to_string();
    let txt = download_text(client, url).await;
    let ver = txt.split(" ").next().unwrap().to_string();
    if os == "macos" {
        Rversion { version: ver, url: Some(MACOS_DEVEL_URI.to_string()) }
    } else if os == "win" {
        Rversion { version: ver, url: Some(WIN_DEVEL_URI.to_string()) }
    } else {
        panic!("Unknown OS: {}", os);
    }
}
...
async fn resolve_oldrel(client: &reqwest::Client, ver: &String, os: &String)
    ...
    let dlurl = get_download_url(version, os);
    Rversion { version: version.to_string(), url: dlurl }
}

async fn resolve_minor(client: &reqwest::Client, ver: &String, os: &String) -> Rversion {
    ...
    let dlurl = get_download_url(&out, os);
    Rversion { version: out, url: dlurl }
}
...
async fn resolve_version(client: &reqwest::Client, ver: &String, os: &String)
    ...
    let dlurl = get_download_url(ver, os);
    Rversion { version: ver.to_string(), url: dlurl }
}
...
fn get_download_url(ver: &String, os: &String) -> Option<String> {
...
async fn download_text(client: &reqwest::Client, url: String) -> String {
