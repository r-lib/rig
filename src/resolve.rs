...
pub async fn resolve_versions(
    ...
    let os = &os;
    let arch = &arch;
    let out: Vec<Result<Rversion, Box<dyn Error>>> = future::join_all(vers.into_iter().map(move |mut ver| {
...
async fn resolve_devel(client: &reqwest::Client,
                       os: &String, arch: &String,
                       linux: Option<LinuxVersion>) -> Result<Rversion, Box<dyn Error>> {
...
async fn download_r_versions(client: &reqwest::Client)
