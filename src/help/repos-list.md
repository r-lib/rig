...
`--r-version` to pick another. Add `--all` to include repositories that
are not enabled by default, and `--raw` to show repository URLs without
resolving the `%` variables in them.
