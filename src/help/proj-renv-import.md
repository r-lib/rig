...
By default rig reads `renv.lock` in the current directory; use `--input` to
point to a different file.

After importing, run [`rig proj lock`](proj.qmd#rig-proj-lock) to solve the dependencies and write
`rproj.lock`.
