...
[`rproj.toml`](proj.qmd) manifest) with rig's built-in solver, for one `(R version,
platform)` target, and write the result as `renv.lock`.

Unlike [`rig proj lock`](proj.qmd#rig-proj-lock) , this only ever solves a single target, since
`renv.lock` has no multi-target concept.
