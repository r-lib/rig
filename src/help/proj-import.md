...
manifest, from it. Fails if `rproj.toml` already exists, use
`--dependencies` to merge dependencies into an existing file instead.
...
`Config/Needs/<name>` depepdencies are parsed specially, an entry that is
not a plain package name (with an optional version requirement) is kept
verbatim as `ref = "..."`. E.g. `tidyverse/tidytemplate` becomes
`tidytemplate = { ref = "tidyverse/tidytemplate" }`. `rig proj export`
writes these back unchanged. Note that only the `test` and `enhances`
groups are installed by rig, so a `Config/Needs/*` group is carried in the
manifest, but not solved or installed by `rig proj lock` and
`rig proj sync`. This behavior will be improved in the future.
