...
`[linking-dependencies]`, or any `[dependency-groups.*]` table (e.g. the
`test` group `--dev` adds to in [`rig proj add`](add.qmd)) — so there is no
need to say which table a package is in.
...
you run [`rig proj lock`](lock.qmd).
