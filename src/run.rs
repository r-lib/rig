...
pub fn run(cmd: OsString, args: Vec<OsString>, _what: &str) -> Result<(), Box<dyn Error>> {
...
#[cfg(target_os = "macos")]
pub fn run_as_user(cmd: String, args: Vec<String>, what: &str) -> Result<(), Box<dyn Error>> {
    let user = get_user()?;
...
fn r_sudo(version: &str, command: &str, user: &User) -> Result<(), Box<dyn Error>> {
    let rbin = get_r_root_for(version).to_string() + "/" + &R_BINPATH.replace("{}", &version_dir_key(version));
    let escaped_command =
...
fn r_sudo(version: &str, command: &str, user: &User) -> Result<(), Box<dyn Error>> {
    ...
    run("su".into(), args, &("R ".to_string() + version))
}
