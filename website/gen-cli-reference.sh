...
# A binary is only usable if it also *runs* here, not just if it is
# executable: `make linux-in-docker` builds into the host `target/` too, so on
# macOS `target/release/rig` is often a Linux ELF binary, which fails to exec
# with a bare status 126 and no message of its own.
runs_here() {
...
RIG="${1:-}"
if [ -z "$RIG" ]; then
  if runs_here "$here/target/release/rig"; then
    RIG="$here/target/release/rig"
  elif runs_here "$here/target/debug/rig"; then
    RIG="$here/target/debug/rig"
  else
    echo "error: no rig binary for this platform in $here/target;" >&2
    echo "       run 'cargo build --release' or pass a path." >&2
    echo "       (\`make linux-in-docker\` leaves a Linux binary there.)" >&2
    exit 1
  fi
elif ! runs_here "$RIG"; then
  echo "error: cannot run $RIG on this platform" >&2
  exit 1
fi
